...
def matching_cascade(
    distance_metric, max_distance, cascade_depth, tracks, detections, track_indices, detection_indices=None
):
...
def gate_cost_matrix(
    ...
    track_indices,
    detection_indices,
    gated_cost=constants.INFINITY_COST,
    only_position=False,
):
    ...
    gating_threshold = constants.chi2inv95[gating_dim]
    measurements = np.asarray([detections[i].to_xyah() for i in detection_indices])
    for row, track_idx in enumerate(track_indices):
        track = tracks[track_idx]
        gating_distance = track.mahalanobis_distance(measurements, only_position)
